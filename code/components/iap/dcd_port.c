...
int dcd_port_init(void)
{
	if(sfud_init() == SFUD_SUCCESS){
		sfud_dev = sfud_get_device_table() + 0;
	}else{
		logError("could not find sfud device:w25q128\n");
        return -1;
	}
	return 0;
}
...
DcdErrCode dcd_port_read(uint32_t addr, uint32_t *buf, size_t size)
{
	...
	if(sfud_dev != NULL)
		sfud_read(sfud_dev, addr, size, (uint8_t *)buf);
...
DcdErrCode dcd_port_write(uint32_t addr, const uint32_t *buf, size_t size)
{
	...
	/* You can add your code under here. */
	if(sfud_dev != NULL)
		sfud_write(sfud_dev, addr, size, (uint8_t *)buf);
...
void dcd_write_example(void)
{
	...
	*/
	dev_gpio_struct dev_w_gpio[]={
		{DCD_GPIO_B,DCD_GPIO_PIN_15,DCD_GPIO_Mode_Out_PP,DCD_GPIO_No_Pull,DCD_GPIO_Speed_HIGH,DCD_GPIO_HIGH},
	};
	...
	if(ret == -1){
		logDebug("dcd device is the latest version=[0x%02X]",DCD_VERSION);
	}else if(ret == -2 ){
		logPrintln("dcd device return lenth error.");
	}else{
		logPrintln("dcd device need to upgrade.");
	}
}
