	...
	dcd_port_write(UPGRADE_DATA_ADDR, &finishbuf, 4);
{
	...
	logInfo("0x%02X,0x%02X,0x%02X,0x%02X", _buff[0], _buff[1], _buff[2], _buff[3]);
}	
#else
...
{
//  if (USART_GetFlagStatus(Ymodem_usart_handle, USART_FLAG_RXDNE) != RESET)
//  {
//    *key = USART_ReceiveData(Ymodem_usart_handle);
//    return 1;
//  }
//  else
//  {
//    return 0;
//  }
	return drv_fifo_data_get(ESERIAL_1, key, 1);
}
...
{
//    USART_SendData(Ymodem_usart_handle, c);
//    while (USART_GetFlagStatus(Ymodem_usart_handle, USART_FLAG_TXDE) == RESET)
//    {
//    }
	
	drv_serial_dma_write(ESERIAL_1, &c, 1);
}
